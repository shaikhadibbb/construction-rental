...
export default function CareersPage() {
  ...
  const handleApply = async () => {
    ...
    if (res.ok) {
      setSent(true)
      pushToast({ title: 'Application submitted', description: 'We will review and get back to you.', variant: 'success' })
    } else {
      setError('Failed to submit. Please try again.')
      pushToast({ title: 'Failed to submit application', variant: 'error' })
    }
    setLoading(false)
  }
  ...
  return (
    <div className="bg-[#080808] text-white">
      {/* Hero */}
      <section className="bg-[#080808] text-white py-20 px-4 text-center border-b border-white/10">
      ...
      {/* Why work with us */}
      <section className="py-14 px-4 bg-[#080808]">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div key={perk.title} className="bg-[#111] border border-white/10 rounded-2xl p-5 text-center shadow-sm">
                <span className="text-3xl block mb-2">{perk.emoji}</span>
                <p className="font-bold text-white text-sm">{perk.title}</p>
                <p className="text-gray-400 text-xs mt-1">{perk.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            {ROLES.map(role => (
              <div key={role.title} className="bg-[#111] border border-white/10 rounded-2xl shadow-sm overflow-hidden">
                <button
                ...
                >
                  <div>
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      <span className="text-xs bg-white/5 text-gray-300 border border-white/10 px-2 py-0.5 rounded-full">📍 {role.location}</span>
                    </div>
                  </div>
                  <span className={'text-2xl transition-transform duration-200 text-gray-500 ' + (selected === role.title ? 'rotate-45' : '')}>+</span>
                </button>

                {selected === role.title && (
                  <div className="px-6 pb-6 border-t border-white/10 pt-4">
                    <p className="text-gray-300 text-sm mb-4">{role.desc}</p>
                    <p className="font-semibold text-white text-sm mb-2">Requirements:</p>
                    <ul className="space-y-1 mb-6">
                      {role.requirements.map(req => (
                        <li key={req} className="text-sm text-gray-300 flex items-start gap-2">
                    ...
                    {sent && selected === role.title ? (
                      ...
                      <div className="bg-white/5 rounded-xl p-5 space-y-3">
                        <p className="font-semibold text-white text-sm">Apply for {role.title}</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            ...
                            <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Your name"
                              className="w-full border border-white/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 bg-[#080808] text-white" />
                          </div>
                          <div>
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-300 mb-1">Tell us about yourself</label>
                          <textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })}
                            placeholder="Licenses held, equipment experience, availability..." rows={3}
                            className="w-full border border-white/15 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none bg-[#080808] text-white" />
                        </div>
      ...
      {/* CTA */}
      <section className="py-16 px-4 bg-[#080808] text-white text-center border-t border-white/10">
        <div className="max-w-xl mx-auto">
          ...
          <a href={`mailto:${ADMIN_EMAIL}`} className="bg-yellow-500 hover:bg-yellow-400 text-gray-900 font-bold px-8 py-3 rounded-xl transition-colors inline-block">
            Send Your CV →
          </a>
        </div>
      </section>
