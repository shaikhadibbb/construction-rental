...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'About ConstructRent | Construction Equipment Rental',
    description: 'Learn how ConstructRent is making equipment rental simple for contractors across India.',
  },
}
    ...
    <div className="bg-white">
      <section className="bg-[#0a1628] relative overflow-hidden py-24 px-4">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-5xl sm:text-6xl font-black text-white mb-6 leading-tight">Built for <span className="text-yellow-500">Builders</span></h1>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto leading-relaxed">ConstructRent was born from a simple idea — renting construction equipment should be as easy as ordering food online. No middlemen, no paperwork, just the right equipment on time.</p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <Link href="/catalog" className="inline-flex items-center gap-2 mt-8 bg-yellow-500 hover:bg-yellow-400 text-[#0a1628] font-black px-6 py-3 rounded-xl transition-all hover:scale-105">Browse Our Equipment →</Link>
            </div>
            <div className="space-y-0">
              {TIMELINE.map((item, i) => (
                <div key={item.year} className="flex gap-5">
                  <div className="flex flex-col items-center">
                    <div className="w-12 h-12 rounded-xl bg-[#0a1628] text-yellow-500 flex items-center justify-center font-black text-sm flex-shrink-0">{item.year}</div>
                    {i < TIMELINE.length - 1 && <div className="w-0.5 flex-1 bg-gray-200 my-2" />}
                  </div>
                  <div className="pb-8 pt-2"><p className="text-gray-600 leading-relaxed">{item.event}</p></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {VALUES.map(value => (
              <div key={value.title} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-yellow-200 transition-all group">
                <div className="w-12 h-12 bg-yellow-50 rounded-xl flex items-center justify-center text-2xl mb-4 group-hover:bg-yellow-500 transition-colors">{value.icon}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {TEAM.map(member => (
              <div key={member.name} className="bg-white border border-gray-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md hover:border-yellow-200 transition-all">
                <div className="w-16 h-16 rounded-2xl bg-[#0a1628] text-yellow-500 flex items-center justify-center font-black text-2xl mx-auto mb-4">{member.initial}</div>
      ...
      <section className="py-24 px-4 bg-[#0a1628] relative overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/catalog" className="bg-yellow-500 hover:bg-yellow-400 text-[#0a1628] font-black px-8 py-4 rounded-xl transition-all hover:scale-105">Browse Equipment →</Link>
            <Link href="/contact" className="bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold px-8 py-4 rounded-xl transition-colors">Contact Us</Link>
          </div>
        </div>
      </section>
    </div>
